...
class Deepfashion2Agg81KpsDataset(JointsDataset):
    ...
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        self._class_to_ind = dict(zip(self.classes, range(self.num_classes))) # name_to_ind
        self._class_to_coco_ind = dict(zip(cats, self.coco.getCatIds())) # name_to_coco_ind
    ...
    def _get_ann_file_keypoint(self):
        ...
        return os.path.join(
            self.root, 'annotations_agg81kps', prefix + '_coco_agg81kps.json'
        )

    def _load_image_set_index(self):
        """ image id: int """
        image_ids = self.coco.getImgIds() # get all images' ids
        return image_ids
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        :return: db entry
        """
        im_ann = self.coco.loadImgs(ids=index)[0] # load the key'Image'
        width = im_ann['width']
        height = im_ann['height']

        annIds = self.coco.getAnnIds(imgIds=index, iscrowd=False) # get all annids of the specified imgid
        objs = self.coco.loadAnns(ids=annIds) # get all anns by all annids in one img according to the specified imgid
        ...
        for obj in objs:
            ...
            self.category_loaded.add(obj['category_id'])
       
        # print(self.category_loaded) # all category_id is loaded
        ...
        for obj in objs:
            ...
            rec.append({
                'image': self.image_path_from_index(index), # 通过index得到当前的image_path
                'center': center,
                'scale': scale,
                'joints_3d': joints_3d,
                'joints_3d_vis': joints_3d_vis, # joint_3d_vis
                'filename': '',
                'imgnum': 0,
    ...
    def _xywh2cs(self, x, y, w, h):
        ...
        according to aspect_ratio: w/h, scale the smaller edge to match the aspect_ratio
        '''
        ...
        scale = np.array(
            ...
            dtype=np.float32) # divided by 200
        if center[0] != -1:
            scale = scale * 1.25
